...
<br><br>
    <form method="Post", action="{{route('commentaire fichier.store')}}">
        @csrf
        <fieldset><legend> Ajout d'un commentaire sur le fichier </legend>
            <div class="row mb-3">
                <label for="nom" class="col-sm-2 col-form-label">utilisateur</label>
                    ...
                    <select class="form-select" name="utilisateur_id" aria-label="Default select example">
                        <option selected>
                            ...
                            @foreach ($commentaire_fichiers as $commentaire_fichier )

                            @if ($commentaire_fichier->utilisateur_id==$utilisateur->id)
            ...
            <div class="form-floating">
                <textarea class="form-control" placeholder="ajouter une observation" name="contenu" id="floatingTextarea" style="height: 100px"> {{$commentaire_fichier->contenu}} </textarea>
